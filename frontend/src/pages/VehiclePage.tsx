...
import { Environment, ContactShadows, useGLTF, Center, PerspectiveCamera, Html } from '@react-three/drei';
...
function getTireColor(pressure: number): string {
  if (pressure <= THRESHOLDS.tire_pressure.critical) return '#ef4444';
  if (pressure <= THRESHOLDS.tire_pressure.low) return '#f59e0b';
  if (pressure >= (THRESHOLDS.tire_pressure.high || 38)) return '#f59e0b';
  return '#22c55e';
}

// 3D Tire Pressure Label
function TireLabel({ position, pressure, label }: { position: [number, number, number]; pressure: number; label: string }) {
  const color = getTireColor(pressure);
...
// Stat Item Component
function StatItem({ label, value, unit, warning = false }: { label: string; value: string | number; unit?: string; warning?: boolean }) {
  return (
    <div className="flex items-center justify-between py-2 border-b" style={{ borderColor: DARK_BORDER }}>
      <span className="text-sm" style={{ color: DARK_TEXT_MUTED }}>{label}</span>
      <span className="text-sm font-mono font-semibold" style={{ color: warning ? '#f59e0b' : DARK_TEXT }}>
        {value}{unit && <span className="text-xs ml-1" style={{ color: DARK_TEXT_MUTED }}>{unit}</span>}
      </span>
    </div>
  );
}
...
export default function VehiclePage() {
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden" style={{ backgroundColor: DARK_BG }}>
      {/* Header */}
      <div className="flex items-center justify-between p-4 pb-2">
        <div>
          <h1 className="text-xl font-bold" style={{ color: DARK_TEXT }}>Tyre Pressure</h1>
          <p className="text-xs" style={{ color: DARK_TEXT_MUTED }}>
            {vehicle?.make || 'Porsche'} {vehicle?.model || '911'} {vehicle?.variant || 'Turbo S'}
          </p>
        </div>
      ...
      <div className="flex-1 flex gap-4 p-4 pt-2 min-h-0">
        ...
        >
          ...
          {/* Legend */}
          <div className="absolute bottom-4 left-4 flex gap-4 text-xs" style={{ color: DARK_TEXT_MUTED }}>
        ...
        <div className="w-64 flex flex-col gap-4">
          ...
          <div 
            ...
            style={{ backgroundColor: DARK_CARD, border: `1px solid ${DARK_BORDER}` }}
          >
            <div className="text-xs uppercase tracking-wider mb-3" style={{ color: DARK_TEXT_MUTED }}>
              Vehicle Stats
            </div>
            <div className="space-y-1">
              ...
              <StatItem 
                ...
                unit="bar"
                warning={data.oil_pressure < 1.5}
              />
              <StatItem 
                label="Fuel Level" 
                value={data.fuel_level.toFixed(0)} 
                unit="%"
                warning={data.fuel_level < 15}
              />
              <StatItem 
                label="Battery" 
                value={data.battery_voltage.toFixed(1)} 
                unit="V"
                warning={data.battery_voltage < 12.2}
              />
            </div>
          </div>
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                { label: 'Front L', value: tires.fl },
                { label: 'Front R', value: tires.fr },
                { label: 'Rear L', value: tires.rl },
                { label: 'Rear R', value: tires.rr },
              ].map((tire) => (
                <div 
                  key={tire.label}
                ...
                >
                  <div className="text-[10px]" style={{ color: DARK_TEXT_MUTED }}>{tire.label}</div>
                  <div className="text-lg font-bold font-mono" style={{ color: getTireColor(tire.value) }}>
                    {tire.value.toFixed(1)}
                  </div>
                </div>
              ))}
            </div>
